...
independent bindDict env (Symbol (prjVariable bindDict -> Just v)) =
...
chooseEnvSub bindDict env (Symbol (prjLambda bindDict -> Just v) :$: a) =
    chooseEnv bindDict env' a
  where
    env' = env
        { inLambda     = True
...
codeMotion bindDict canShr a
  ...
  where
    ...
    share (SomeAST b) = do
        ...
        v  <- get; put (v+1)
        let x = Symbol (injVariable bindDict b v)
        body <- codeMotion bindDict canShr $ substitute b x a
        return
            $   Symbol (injLet bindDict body)
            :$: b'
            :$: (Symbol (injLambda bindDict body v) :$: body)

    descend :: AST dom b -> State VarId (AST dom b)
    descend (f :$: a) =
        liftM2 (:$:) (descend f) (codeMotion bindDict canShr a)
    descend a = return a
...
defaultBindDict = BindDict
    ...
    , injVariable = \_ v -> inject (Variable v `withContext` ctx)
    , injLambda   = \_ v -> inject (Lambda   v `withContext` ctx)
    , injLet      = \_   -> inject (letBind ctx)
    }
  where
    ctx = Proxy :: Proxy ctx
